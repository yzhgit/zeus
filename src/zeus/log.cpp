...
//
// File: log.cpp - Logging infrastructure
//
...
#include <zeus/backtrace.h>
#include <zeus/log.h>
#include <zeus/logging.h>
 ...
 * levels.
 *
 * The levels are configurable through the ZEUS_LOG_LEVELS environment
 ...
 * log file by setting the ZEUS_LOG_FILE environment variable to the name
 ...
 * ZEUS_LOG_NO_COLOR environment variable to disable coloring.
 */
Logger::Logger()
{
    bool color = !utils::secure_getenv("ZEUS_LOG_NO_COLOR");
 ...
 *
 * If the ZEUS_LOG_FILE environment variable is set, open the file it
...
{
    const char *file = utils::secure_getenv("ZEUS_LOG_FILE");
    if (!file)
        return;
 ...
 *
 * The log levels are stored in the ZEUS_LOG_LEVELS environment variable
...
{
    const char *debug = utils::secure_getenv("ZEUS_LOG_LEVELS");
    if (!debug)
        return;
